(ns rets_mls.main
  "Kotodama WASM entrypoint for the Rets_mls clean-room actor (L4) — Clojure port.
  ...
  facts (`rets_mls.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "rets_mls")
(def tier "L4")
...
(def entity-specs
  [{:entity "Listing"     :plural "listings"     :id-prefix "retsmls_lis"
   ...
   {:entity "Tenant"      :plural "tenants"      :id-prefix "retsmls_ten"
...
(defn emit-facts
  "EAVT facts for one record: {\"rets_mls.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "rets_mls-compat" :tier tier :entities entities} 200])
...
(defn start! [] :rets_mls-compat/ready)
